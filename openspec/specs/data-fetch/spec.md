...
### Requirement: Tickers overview uses SQLite aggregates, no DuckDB

The system SHALL serve `/api/tickers` from the SQLite `bars` table
by aggregating `(figi, COUNT(*), MIN(ts), MAX(ts))` joined to
`instruments`. No DuckDB connection is opened for this endpoint.

#### Scenario: Tickers overview latency under 100 ms

- GIVEN 3,699 figis with 2.36M raw bar rows in SQLite `bars`
