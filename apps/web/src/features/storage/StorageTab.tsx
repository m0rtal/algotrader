  ...
  return (
    <div className="p-4">
      <div className="grid grid-cols-4 gap-2 mb-3.5">
        <Card label="Всего баров" value={totalBars.toLocaleString('ru')} sub={`${data.length} тикеров × 5 лет`} />
        <Card label="Размер на диске" value={`${(totalSize / 1024 / 1024).toFixed(1)} MB`} sub="parquet сжатый" />
        <Card
          label="Период"
          value="2021-09 → сегодня"
          sub="1 247 торговых дней"
        />
        <Card label="Полнота данных" value="99.2%" sub={`гэпы: ${data.reduce((s, t) => s + t.gaps, 0)} дней`} />
      </div>
        ...
        <table className="w-full text-[11px]">
          <thead>
            <tr className="bg-surface">
              {['Тикер', 'Баров', 'Период', 'Размер', 'Гэпы', 'Last update', 'Δ 5д', 'Spark'].map((h) => (
                <th
                  key={h}
          ...
          <tbody>
            {data.map((t) => (
              ...
              >
                <td className="px-2 py-1.5 mono border-b border-border-soft font-medium">{t.symbol}</td>
                <td className="px-2 py-1.5 mono border-b border-border-soft">{t.bars.toLocaleString('ru')}</td>
