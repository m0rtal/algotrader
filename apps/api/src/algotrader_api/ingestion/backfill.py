...
class BackfillRunner:
    ...
    async def backfill_from_moex(
        ...
        *,
        ...
        # Concurrency: process up to 5 figis in parallel. Sequential
        # walks make 3802 figis take many hours; rate-limited Tinkoff
        # fallback path is the bottleneck. 5 is well under the 600/min
        # Tinkoff limit even with retry storms.
        _sem = asyncio.Semaphore(5)
        # Per-figi budget: skip the Tinkoff fallback after 90s so a
        # single sanctions-delisted ticker can't stall the chain.
        _figi_timeout_s = 90
        ...
        results = await asyncio.gather(
            *[_process_one(inst) for inst in instruments],
        ...
        for i, res in enumerate(results):
            if isinstance(res, Exception):
                ...
                continue
            # res is int here (Exception branch handled above)
