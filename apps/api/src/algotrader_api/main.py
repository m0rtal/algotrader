...
def _flag_orphan_ok_rows(sqlite_path: str) -> int:
    """Mark `instrument_metadata` rows as error if no bars exist.
    ...
    failed to handle the SDK's pre-converted dict candles and bailed
    early, or because the row was written to a `null` legacy parquet
    file. Those rows lie to the pending counter (which counts them as
    ...
    has zero bars in the SQLite `bars` table gets flipped to `error`
    so the next backfill run will retry it.
    """
    ...
    have_bars = {r["figi"] for r in have_rows if r["figi"]}
    orphan_ids = [
        figi for r in rows if (figi := r["figi"]) and (ticker := r["ticker"])
        if ticker not in have_bars and figi not in have_bars
    ]
    if not orphan_ids:
        return 0
...
def create_app() -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        # but the corresponding bars table has no rows (or has the
        ...
        try:
            _flag_orphan_ok_rows(settings.sqlite_path)
        except Exception as e:  # noqa: BLE001
            logger.warn("backfill.orphan_check_failed", error=str(e))

        # Seed synthetic bars only when:
        # - no bars rows exist yet, AND
        # - ALGOTRADER_SYNTH_SEED is set OR the Tinkoff token file is missing
        #   (dev environment without secrets).
        bar_count = sqlitedb.execute(
            settings.sqlite_path, "SELECT COUNT(*) FROM bars", ()
        )[0]["COUNT(*)"]
        if bar_count == 0 and should_seed_synth():
            logger.info("seed.start", sqlite_path=settings.sqlite_path)
            n = seed_bars_sqlite(settings.sqlite_path)
            logger.info("seed.done", bars_written=n)
        elif bar_count == 0:
            logger.info(
                "seed.skipped",
                reason="no bars but token file present; worker will populate on next cron tick",
            )
